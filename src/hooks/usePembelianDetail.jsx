...
const pembeliandetailIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getPembelianDetailList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getPembelianDetailById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createPembelianDetail(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updatePembelianDetail( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deletePembelianDetail(id, token);
  },
};
...
export function usePembelianDetail(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await pembeliandetailIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await pembeliandetailIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await pembeliandetailIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await pembeliandetailIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await pembeliandetailIpc.getById(id);

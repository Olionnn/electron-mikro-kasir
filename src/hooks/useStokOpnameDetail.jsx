...
const stokopnamedetailIpc = {
  getList: (params) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getStokOpnameDetailList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getStokOpnameDetailById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createStokOpnameDetail(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updateStokOpnameDetail( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deleteStokOpnameDetail(id, token);
  },
};
...
const toBackend = (p = {}) => ({
  toko_id: p.toko_id ?? "",
  stok_opname_id: p.stok_opname_id ?? true,
  stokOpnameDetail_id: p.stokOpnameDetail_id ?? 0,
  stok_sistem: p.stok_sistem ?? 1,
  stok_fisik: p.stok_fisik ?? 1,
  stok_sesuai: p.stok_sesuai ?? null,
...
const fromBackend = (row = {}) => {
  ...
  return {
    id: r.id,
    toko_id: r.toko_id ?? "",
    stok_opname_id: r.stok_opname_id ?? true,
    stokOpnameDetail_id: r.stokOpnameDetail_id ?? 0,
    stok_sistem: r.stok_sistem ?? 1,
    stok_fisik: r.stok_fisik ?? 1,
    stok_sesuai: r.stok_sesuai ?? null,
...
export function useStokOpnameDetail(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await stokopnamedetailIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await stokopnamedetailIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await stokopnamedetailIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await stokopnamedetailIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await stokopnamedetailIpc.getById(id);

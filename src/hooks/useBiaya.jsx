...
const biayaIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getbiayaList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getbiayaById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createbiaya(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updatebiaya( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deletebiaya(id, token);
  },
};
...
export function useBiaya(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await biayaIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await biayaIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await biayaIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await biayaIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await biayaIpc.getById(id);

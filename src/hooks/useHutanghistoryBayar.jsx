...
const hutanghistorybayarIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getHutangHistoryBayarList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getHutangHistoryBayarById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createHutangHistoryBayar(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updateHutangHistoryBayar( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deleteHutangHistoryBayar(id, token);
  },
};
...
export function useHutangHistoryBayar(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await hutanghistorybayarIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await hutanghistorybayarIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await hutanghistorybayarIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await hutanghistorybayarIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await hutanghistorybayarIpc.getById(id);

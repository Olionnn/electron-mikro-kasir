...
const stokopnameIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getStokOpnameList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getStokOpnameById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createStokOpname(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updateStokOpname( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deleteStokOpname(id, token);
  },
};
...
export function useStokOpname(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await stokopnameIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await stokopnameIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await stokopnameIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await stokopnameIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await stokopnameIpc.getById(id);

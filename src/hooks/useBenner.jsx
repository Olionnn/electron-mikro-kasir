...
const bennerIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getbennerList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getbennerById(id, token);
  },
  create: (data) => { 
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createbenner(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updatebenner( id, data )},
  remove: (id) => window.electronAPI.deletebenner(id, token ),
};
...
export function usBenner(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await bennerIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await bennerIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await bennerIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await bennerIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await bennerIpc.getById(id);

...
const piutanghistorybayarIpc = {
  getList: (params) => {
    ...
    return window.electronAPI.getPiutangHistoryBayarList(params, token);
  },
  getById: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.getPiutangHistoryBayarById(id, token);
  },
  create: (data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.createPiutangHistoryBayar(data, token);
  },
  update: (id, data) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.updatePiutangHistoryBayar( id, data, token);
  },
  remove: (id) => {
    const token = getAccessToken();
    if (!token) return Promise.resolve({ success: false, error: "Unauthorized" });
    return window.electronAPI.deletePiutangHistoryBayar(id, token);
  },
};
...
export function usePiutangHistoryBayar(initialParams = DEFAULT_PARAMS) {
  ...
  const refresh = useCallback(async (nextParams = {}) => {
    ...
    try {
      ...
      const res = await piutanghistorybayarIpc.getList(paramsRef.current);
  ...
  const create = useCallback(
    async (payload) => {
      const res = await piutanghistorybayarIpc.create(toBackend(payload));
  ...
  const update = useCallback(
    async (id, payload) => {
      const res = await piutanghistorybayarIpc.update(id, toBackend(payload));
  ...
  const remove = useCallback(
    async (id) => {
      const res = await piutanghistorybayarIpc.remove(id);
  ...
  const getById = useCallback(async (id) => {
    const res = await piutanghistorybayarIpc.getById(id);

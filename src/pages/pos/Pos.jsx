...
const Pos = () => {
  ...
  const submitCustomer = () => {
    ...
    if (editingCust) {
      ...
      const newId = customers.length
        ? Math.max(...customers.map((c) => Number(c.id))) + 1
        : 1;
      const row = {
        id: newId,
        toko_id: 1,
      ...
      actions: [
        {
          ...
          title: "Pesanan",
          className:
            "relative p-3 bg-orange-400 text-white rounded-full w-12 h-12",
          icon: (
            <>
  ...
  const renderSelectedIcon = () => {
    switch (selectedIcon) {
      case "search":
        return <IoSearch size={30} className="text-gray-500" title="Search" />;
      case "barcode":
        return (
          <IoBarcode size={30} className="text-gray-500" title="Barcode" />
        );
      case "receipt":
        return (
          <IoReceiptOutline
  ...
  const loadHistoryToCart = (trx) => {
    setCart(
      trx.items.map((it) => ({ ...it, hargaJualOverride: it.hargaJual }))
    );
  ...
  const handleCheckoutClick = () => {
    ...
    const orderData = {
      ...
      customer: { 
        ...
        poin: customers.find(c => c.id === currentCustomer?.id)?.poin ?? 0 
      },
      items: cart,
      total: totalPrice,
  ...
  const afterGlobalDisc = useMemo(() => {
    ...
    return globalDiscount.type === "pct"
      ? Math.max(
          0,
          subtotalItems - (subtotalItems * globalDiscount.value) / 100
        )
      : Math.max(0, subtotalItems - globalDiscount.value);
  }, [subtotalItems, globalDiscount]);

  const taxAmountUI = useMemo(
    () => Math.round(afterGlobalDisc * (Number(globalTax?.pct || 0) / 100)),
    [afterGlobalDisc, globalTax]
  );
      ...
      <div className="w-3/5 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="p-2">
          <div className="flex items-center gap-3">
            ...
            <div
              className="flex items-center flex-1 border rounded-2xl px-4 py-2 bg-gray-50"
              title="asd"
            >
              {renderSelectedIcon()}
              <input
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button className="px-4 py-2 rounded-full border bg-white hover:bg-gray-50 text-sm">
              Semua
            </button>
            {selectedIcon === "receipt" ? (
              <></>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  Rp {formatCurrency(subtotalItems)}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-medium text-amber-700">
                  - Rp{" "}
                  {formatCurrency(Math.max(0, subtotalItems - afterGlobalDisc))}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl p-6">
            ...
            <div className="space-y-4">
              {promotionEvents.map((promo) => (
                <div
                  key={promo.id}
                  className="flex items-center gap-4 border rounded-lg p-3 hover:shadow-md transition bg-white"
                >
                  <img
                    src={promo.img}
                    alt={promo.name}
          ...
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-500">
                  Pilih pelanggan untuk transaksi ini atau tambah pelanggan
                  baru.
                </div>
              </div>
              <button

...
export default function KategoriBarangPage({ variant = "A" }) {
    ...
    () => [
      {
        ...
        onClick: openTambah,
        className:
          "inline-flex items-center gap-2 - px-3 py-2 rounded-lg hover:opacity-95",
        icon: <MdAdd size={20} />,
        style: {
          background:
            "linear-gradient(90deg, var(--primary-700), var(--primary-800))",
        },
      },
      {
        type: "button",
  ...
  const addKategori = useCallback(
    async (e) => {
      ...
      try {
        ...
        setAlert({
          type: "error",
          message: error?.message || "Gagal menambah kategori",
        });
      } finally {
        setSaving(false);
      }
  ...
  const toggleStatus = useCallback(async () => {
    ...
    try {
      ...
      refresh();
      setAlert({ type: "success", message: "Status kategori berhasil diubah" });
    } catch (error) {
      setAlert({
        type: "error",
        ...
        <div className="lg:col-span-2">
          <div className="sticky top-4">
            {!selected ? (
              ...
              <PreviewCard selected={selected} toggleStatus={toggleStatus} doRemove={doRemove} />
            )}
            <div className="mt-4">
              <button
                onClick={openTambah}
...
function PreviewCard({ selected, toggleStatus, openEdit, doRemove }) {
  return (
